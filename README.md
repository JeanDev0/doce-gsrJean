# Doce GSR

Este é o modelo de DOCumentação Estudante (DOCE) oficial para disciplina Gerência e Segurança de Redes de Computadores (GSR).

Um membro de seu grupo do projeto integrador deve importar este repositório na instância do Gitea instalado na Mange[^1] e eventualmente fazer um clone para um repositório do GitHub[^2].

1. Abra seu navegador web preferido;
2. Na barra de endereço[^3] do navegador, acesse o endereço [jurandy.net/gitea](https://jurandy.net/gitea);
2. Autentique-se com sua matrícula e senha do [SUAP](https://suap.ifrn.edu.br/);
3. 


[^1]: Disponível em <https://gitea.mange.ifrn.edu.br>
[^2]: Disponível em <https://github.com>
[^3]: Pressione as teclas <kbd>Ctrl</kbd>+<kbd>L</kbd>

